...
def galerkin_method_2(coefficients, f, num_of_basis_functions, a, b):
    ...
    def resudial_part_diff(a):
        return lambda x: sum(coefficients[i](x) * numerical_diff(basis[a], x, i) for i in range(len(coefficients)))
    ...
    right_side = [numerical_integration(lambda x: (f(x) - part_diffs[0](x)) * basis[i](x), a, b) for i in range(1, num_of_basis_functions)]
