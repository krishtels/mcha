...
from help_func import build_function_from_basis, func_for_substantiation, get_basis_system, numerical_diff
import numpy

def ref(subs):
    y = -0.3655 * (1 - x ** 2) + 0  * x * (1 - x ** 2) + 0.0326 * x ** 2 * (1 - x ** 2)
    print(y)
...
def collocation_method(coefficients, f, collocation_points):
    ...
    def resudial_part_diff(a):
        return lambda d: sum(coefficients[i](d) * numerical_diff(basis[a], d, i) for i in range(len(coefficients)))

    matrix = [[resudial_part_diff(i)(point) for i in range(1, num_of_basis_functions)] for point in collocation_points]
    print(matrix)
    right_side = [f(point) - resudial_part_diff(0)(point) for point in collocation_points]
